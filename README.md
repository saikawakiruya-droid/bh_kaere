...
| `core/metrics.py` | ループ数・行き止まり数の計数（`verification/` の評価が使用） |
| `core/errors.py` | 例外クラスの階層 |
| `validation/config.py` / `validation/options.py` | 必須キーの検証／任意キーの処理 |
| `mazegen.py` | 再利用可能な単一ファイルモジュール（仕様 VI の必須成果物）。**生成の全工程**＝掘り（バックトラッカー）＋「42」記号の配置＋プレイアブル整形（braiding）を担う |
...
| `mazegen-2.0.0-*.whl` / `.tar.gz` | ビルド済みパッケージ（仕様 VI：`mazegen-*` をリポジトリ直下に配置） |
...
> 注記：現状の生成アルゴリズムは `mazegen` のバックトラッカーのみです。`ALGORITHM` キーは将来 Prim 法や Kruskal 法などを追加できる拡張点として受理・検証されます（現時点で選択できるのは `backtracker` だけです）。

### プレイアブル盤面（`PERFECT=False`、仕様 IV.4, v2.2）

バックトラッカーが全域木を作った後、`mazegen` の braiding が 3 フェーズで盤面を整形します：(1) 行き止まりの削減、(2) 四隅と中央の通路化（開口 2 つ以上）、(3) 独立ループ 2 本以上の保証。いずれの壁開放も 3×3 の開放領域を作らず、「42」予約セルには触れません。
...
四隅と中央のセルは `(0,0)`／`(WIDTH-1,0)`／`(0,HEIGHT-1)`／`(WIDTH-1,HEIGHT-1)` と、中央 `(WIDTH//2, HEIGHT//2)` です（整数除算）。幅・高さが偶数のときは中央 2 セルのうち大きい添字側（右／下寄り）が選ばれます（例：`WIDTH=20` なら `x=10`）。生成側は自己完結モジュール `mazegen`、検証側は `core/maze.py` の `playable_corridors` がそれぞれ同じ定義を持ちます（`mazegen` は単一ファイル提出物として意図的に自前実装。両者は同一定義を保つ）。
...
**生成は自己完結モジュール `mazegen`、評価は `core`** という分担です。生成の全工程（掘り＋「42」記号＋プレイアブル整形）は `mazegen` が担い、`a_maze_ing.py` は設定を読み取って `spec`（dict）に変換し、`mazegen.generate_mazes` に渡して生成、結果を `core`／`verification` で評価します。

```python
from mazegen import generate_mazes
from core.maze import Maze, solve, solution_cells

# spec（dict）のリストを渡して生成。戻り値は spec と同順の壁グリッドのリスト
grid = generate_mazes([{
    "width": 25, "height": 20, "seed": 42, "perfect": True,
    "entry": (0, 0), "exit": (24, 19),   # 「42」記号は自動配置（省略可）
}])[0]

# 壁コードにアクセス（bit0=N、bit1=E、bit2=S、bit3=W、閉じている場合は 1）
code = grid[0][0]

# 評価・求解は core 側で（グリッドを Maze に載せ替え）
maze = Maze(25, 20, cells=grid)
path = solve(maze, (0, 0), (24, 19))           # 例: "EESS..."
cells = solution_cells(maze, (0, 0), (24, 19)) # 経路上にあるセルの集合
```

`spec` のキー: `width`・`height`（必須）、`perfect`（既定 True）、`seed`、`rng`（注入する `random.Random`。`seed` より優先）、`entry`、`exit`、`sign`（既定 `"42"`。`""`／`None` で無効）、`reserved`（追加のマスキング座標）、`min_loops`（既定 2。`perfect=False` 時）、`gap`（記号の桁間隔）。`perfect=False` を渡すと四隅・中央の通路化と独立ループ保証まで含むプレイアブル盤面が生成されます。

主なパッケージ:

- `mazegen.py`：**生成の全工程**（`generate_mazes` — 掘り＋「42」記号＋braiding）。`supported_sign_chars()` で記号に使える文字を取得
- `core/maze.py`：壁の表現（`Maze`）、`bfs_distances`、`solve`、`solution_cells`、`playable_corridors`
- `core/metrics.py`：`count_loops`、`count_dead_ends`（`verification` の評価が使用）
...
#### 各モジュールの使い方（生成 → 求解 → 検証 → 出力）

主要モジュールを組み合わせた、パイプライン一巡のエンドツーエンド例です。`a_maze_ing.py` を介さずに、ライブラリとして直接呼び出せます。

```python
from mazegen import generate_mazes, supported_sign_chars
from core.maze import Maze, solve, solution_cells
...
entry, exit_ = (0, 0), (24, 19)

# 1) 生成（mazegen）：spec のリストを渡し、spec と同順の壁グリッドを得る
grid = generate_mazes([{
    "width": 25, "height": 20, "seed": 42, "perfect": True,
    "entry": entry, "exit": exit_,   # 「42」記号は既定で自動配置（省略可）
}])[0]

# 2) 評価用に Maze へ載せ替え（core）。以降 grid ではなく Maze を使う
maze = Maze(25, 20, cells=grid)

# 3) 求解（core）：最短経路文字列と、経路上のセル集合
solution = solve(maze, entry, exit_)          # 例: "EESS..."（到達不能なら None）
cells = solution_cells(maze, entry, exit_)    # 経路上セルの集合（描画に使える）

# 4) 検証（verification）：問題点のリストを返す（空なら合格）
problems = validate(maze, entry, exit_, perfect=True, solution=solution)
if problems:
    print("\n".join(problems))

# 5) 出力（output）：16 進形式の文字列化／ファイル書き込み
text = format_maze(maze, entry, exit_, solution)   # 文字列として取得
write_maze("maze.txt", maze, entry, exit_, solution)  # そのままファイルへ

# 6) ASCII 表示（output）：経路や「42」記号を重ねて描画
print(render_ascii(maze, entry, exit_, path=cells))

# 補助：記号に使える文字の集合（現状 {"2", "4"}）
print(sorted(supported_sign_chars()))
```
...
| モジュール | 呼び出し | 役割 |
|------|------|------|
| `mazegen` | `generate_mazes(specs)` | 生成の全工程（掘り＋「42」＋braiding）。戻り値は `grid[y][x]` の壁コード 2 次元配列 |
| `mazegen` | `supported_sign_chars()` | `SIGN` に使える文字集合（`frozenset`） |
| `core.maze` | `Maze(w, h, cells=grid)` | 壁グリッドを評価用オブジェクトに載せ替え |
| `core.maze` | `solve(maze, entry, exit_)` | 最短経路（`N/E/S/W` 文字列、到達不能で `None`） |
| `core.maze` | `solution_cells(maze, entry, exit_)` | 経路上セルの集合 |
| `verification.verifier` | `validate(maze, entry, exit_, *, perfect=, solution=, reserved=, playable=)` | 構造検証。問題文字列のリスト（空＝合格） |
| `output.writer` | `format_maze(...)` / `write_maze(path, ...)` | 16 進出力形式の文字列化／ファイル書き込み |
| `output.ascii_display` | `render_ascii(maze, entry, exit_, path=)` | ASCII 図の文字列化 |

> `validate()` の `playable=True` は仕様 v2.2 のプレイアブル要件（四隅・中央の通路化、独立ループ 2 本以上、行き止まり僅少）を検査するオプトインです。`PERFECT=False` の盤面を検証するときに指定し、CLI（出力ファイル検証）では既定で無効です。予約セル（「42」）を検証に含める場合は `reserved=` に座標集合を渡します。

`mazegen.py` は **core などに一切依存しない**自己完結の単一ファイルなので、`pip` パッケージとしても配布できます。ビルド済み成果物（`mazegen-2.0.0-py3-none-any.whl` と `mazegen-2.0.0.tar.gz`）はリポジトリ直下にあります。MIT ライセンス（`LICENSE.md`）で公開しており、後続の 42 プロジェクトでの再利用が明示的に許可されています。

```python
# インストール済みホイールから、他プロジェクトでそのまま再利用
from mazegen import generate_mazes

grids = generate_mazes([{"width": 20, "height": 15, "seed": 1, "perfect": False}])
grid = grids[0]  # 4 ビットの壁コードからなる 2 次元配列（プレイアブル盤面）
```
...
- **メンバーと担当:**
  - **naarai**：コア機能と生成。`core/maze.py`（壁モデル、BFS／求解）、および生成の全工程（掘り＋「42」記号＋プレイアブル整形）を集約した再利用可能な `mazegen` パッケージを担当。
