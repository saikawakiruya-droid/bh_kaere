...
迷路パイプラインは役割ごとに小さなディレクトリへ分かれており、各モジュールは
単体で import できる（詳細は後述「再利用できるコード」参照）。**validation**
（迷路生成前の設定ファイル入力チェック）と **verification**（生成後の迷路構造
チェック）は別々の工程なので別ディレクトリにしている：

| ディレクトリ | 役割 |
|------|------|
| `core/` | `maze.py`（壁表現、BFS、最短経路）、`metrics.py`（ループ数・デッドエンド数の計測、braiding と verification が共用）、`errors.py`（例外階層） |
| `validation/` | 設定ファイルの**入力チェック**：`config.py`（必須キー）、`options.py`（任意キー） |
| `generation/` | `initializer.py`（「42」サイン予約+初期マップ）、`backtracker.py`（recursive backtracker アルゴリズム本体）、`generator.py`（アルゴリズム選択レジストリ） |
| `braiding/` | `braiding.py`（不完全迷路化・プレイアブル盤面変換、`PERFECT=False`） |
| `verification/` | 生成後の**迷路構造チェック**：`verifier.py`（ライブラリ: `validate()` + 条件チェック）、`cli.py`（単独CLI: `python -m verification.cli <file>`） |
| `output/` | `writer.py`（spec IV.5 ファイル出力）、`ascii_display.py`（ASCII 描画）、`display.py`（表示モード選択レジストリ） |
...
- [ ] `python3 -m verification.cli maze.txt` が `OK` を表示。
...
- [ ] 迷路にループがある（`braiding/braiding.py`）。経路表示をトグルすると
      別ルートが見え、`python3 -m verification.cli maze.txt` が `OK` を返す。
- [ ] 完全連結：「42」以外に孤立セルがない。
...
### 任意キー（`validation/options.py` が処理）
...
のみ）。設計上は `generation/backtracker.py` の隣に新しいアルゴリズムを追加し、
`generation/generator.py` の `ALGORITHMS` に Prim 法や Kruskal 法などを登録して
直接選べるようにしてある。
...
`braiding/braiding.py` が3フェーズで整形する：

1. **デッドエンド削減** — 各行き止まりが壁を1枚開けて他とつながり、追われる
   プレイヤーが罠にかかりにくくなる。
2. **通路化の強制** — 四隅と中央（`generation/initializer.py` が「42」から除外
...
これは設定ファイルの**入力チェック（validation）とは別の工程**（前述「設定
ファイルの構造と形式」参照）：validationは迷路を作る前に設定の*入力*を
チェックするもの、verificationはすでに*作られた*迷路の構造をチェックする
もの。専用の verifier は、生成された迷路が仕様 IV.4 の条件を満たすことを
確認する：
...
ライブラリ（`verification.verifier.validate()`、`playable` フラグ付き）
としても、単独CLI（`python3 -m verification.cli <file>`、
`verification/cli.py`）としても使える。CLI は出力ファイルが意図した
...
パイプライン（初期マップ・生成・braiding・検証・出力・表示）は、1つに束ねる
パッケージではなく役割ごとの小さなディレクトリに分かれている。各モジュールは
`a_maze_ing.py` に依存しないため、**単体のモジュールだけを import して
使える** — その処理だけ必要なら、そのファイル1つ（と唯一の依存先である
`core/maze.py` / `core/errors.py`）を別プロジェクトにコピーすればよい。
各モジュール自身の docstring に "Standalone usage" の例がある。
...
# braidingだけ（PERFECT=Falseのプレイアブル盤面変換）
...
- `core/` — `maze.py`（コアの `Maze` クラス：`open_wall`、`bfs_distances`、`solve`、`solution_cells`）、`metrics.py`（`count_loops`、`count_dead_ends`。braiding と verification が共用）、`errors.py`（例外階層）
- `validation/` — `config.py`（必須キーの入力チェック）、`options.py`（任意キーの入力チェック）
- `generation/` — `initializer.py`（「42」サイン予約+初期マップ）、`backtracker.py`（recursive backtracker アルゴリズム本体）、`generator.py`（アルゴリズム選択レジストリ：`get_algorithm`、`algorithm_names`）
- `braiding/` — `braiding.py`（不完全迷路化・プレイアブル盤面変換）
- `verification/` — `verifier.py`（`validate()` + 条件チェック）、`cli.py`（単独出力ファイルCLI）
- `output/` — `writer.py`（出力ファイル書き出し、16進形式）、`ascii_display.py`（ASCII 描画）、`display.py`（表示モード選択レジストリ：`get_display_mode`、`display_names`）
...
- **メンバーと役割：**
  - **naarai** — コア & 生成：`core/maze.py`（壁モデル、BFS/求解）、
    `generation/backtracker.py` / `braiding/braiding.py`（recursive backtracker、
    不完全盤面）、および `mazegen` 再利用パッケージ。
  - **ksadayas** — 入出力 & 検証：`validation/config.py` /
    `validation/options.py` / `core/errors.py`（設定解析）、
    `output/writer.py` / `output/ascii_display.py`、
    `verification/verifier.py`（ASCII + 対話）、テスト、ドキュメント。
  - _(上の分担は実際の作業分担に合わせて調整すること。)_
- **計画と変遷：** 「求解 → 初期化(42) → 生成 → 出力 → 表示 → 検証」の順に
  段階的に構築し、機能ごとに小さくコミットした。
- **うまくいった点：** 壁表現を1か所（`core/maze.py`）に集約したことで、
  生成・表示・出力・検証の間の食い違いを防げた。例外を致命/非致命に階層化
  したことでエラーを区別しやすくなった。`PERFECT=False` モードは仕様 v2.2 の
  プレイアブル盤面規則（四隅と中央の開通、独立ループ2本以上、デッドエンド
  稀）を満たし、`braiding/braiding.py` で強制し `verification/verifier.py` で
  確認している。
